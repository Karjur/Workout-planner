<template>
    <div class="row g-3 align-items-center mt-2">
        ...
        <div class="col-auto" v-if="selectList && selectLabel">
            <!-- {{modelValue}} -->
            <select class="form-select" :value="modelValue"  @change="inputAction($event.target.value)">
        ...
        <div class="col-auto" v-else>
            <input :type="typeof(modelValue) == 'string' ? 'text' : 'number'" :value="modelValue" class="form-control" @input="inputAction($event.target.value)">
        </div>
    </div>
</template>
<script lang="ts">
