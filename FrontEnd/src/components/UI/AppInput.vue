  ...
  <div class="row g-3 align-items-center mt-2">
    ...
    <div class="col-auto" v-if="selectList && selectLabel">
      ...
      <select
        class="form-select"
        :value="modelValue"
        @change="(e:any) => inputAction(e.target?.value)"
      >
        <option>{{ selectLabel }}</option>
        <option v-for="item in selectList" :key="item.key" :value="item.value">
          {{ item.key }}
        </option>
      </select>
    </div>
    <div class="col-auto" v-else>
      <input
        ...
        :value="modelValue"
        class="form-control"
        @input="(e:any) => inputAction(e.target?.value)"
      />
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  methods: {
    inputAction(value: any) {
      if (typeof value == 'string') this.$emit('update:modelValue', value);
